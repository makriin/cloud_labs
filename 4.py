...
def get_public_ip():
    ...
    k = 1
    for i in instances:
        print(k, "::", i["instance_id"])
        k += 1
    while True:
        instance_id = int(input("Enter number of instance: "))
        if instance_id > len(instances) or instance_id <1:
            print("Wrong number!")
        else: break
    print("IP::", instances[instance_id - 1]["public_ip"])
    return instances[instance_id - 1]["public_ip"]

def get_public_ip1(instance_id):
    ip = []
    ec2_client = boto3.client("ec2", region_name="us-east-1")
...
def get_running_instances():
    ...
    for reservation in reservations:
        for instance in reservation["Instances"]:
            ...
            instances.append({"instance_id": instance_id, "instance_type": instance_type, "public_ip": public_ip,
...
def ssh():
    tmp = get_public_ip1("i-0537588b9f590e3e6")
